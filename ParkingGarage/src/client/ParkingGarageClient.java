...
public class ParkingGarageClient {
	...
	static ParkingGarageImpl garage;
	...
	public static void main(String[] args) {
		...
		try {
			garage = (ParkingGarageImpl) Naming.lookup("rmi://" + args[0] + ":" + args[1] + "/ParkingGarage");
		...
		garage = new ParkingGarageImpl(maxOccupancy);
	...
	private static void evaluateUserChoice(String choice) {
		...
		case "3":
			...
			if (exit.attemptCheckoutCar(ticketNumber)) {
				...
				LocalDateTime today = LocalDateTime.of(ldt.getYear(), ldt.getMonthValue(), ldt.getDayOfMonth(),
						ldt.getHour(), 0);
				...
				System.out.println(
						"Sorry but the ticket number was not found. Please try again or ask for an Administrator.");
			}
